...
DEMO_FAST_KEYWORDS = {"x", "twitter", "x.com", "youtube"}
...
def detect_distraction(window_title: str) -> DistractionSignal | None:
    ...
    if re.search(r"(^|[\s/|:()［］\[\]-])x($|[\s/|:()［］\[\]-])", normalized):
        return DistractionSignal("social", "x", window_title, demo_fast=True)
    for category, keywords in DISTRACTION_KEYWORDS.items():
        for keyword in keywords:
            if keyword.casefold() in normalized:
                return DistractionSignal(
                    category,
                    keyword,
                    window_title,
                    demo_fast=keyword.casefold() in DEMO_FAST_KEYWORDS,
                )
    return None
